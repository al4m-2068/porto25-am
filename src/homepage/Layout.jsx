...
export default function Layout() {
    ...
    // --- FUNGSI DIRECT EMAIL KE GMAIL WEB (TAB BARU) ---
    const handleSubmitEmail = (e) => {
        if (e) e.preventDefault();
        
        // Validasi agar tidak kirim teks kosong atau cuma draf awal
        if (!message.trim() || message.trim() === "Dear Albar,") return;

        const emailTujuan = "albarabdulmaalik@gmail.com";
        const subject = encodeURIComponent("Inquiry from Portfolio");
        const body = encodeURIComponent(message);

        // Link compose Gmail Web
        const gmailWebUrl = `https://mail.google.com/mail/u/0/?fs=1&to=${emailTujuan}&su=${subject}&body=${body}&tf=cm`;

        // Membuka tab baru langsung ke Gmail Web compose
    ...
    return (
        <>
            ...
            <footer 
                ref={footerRef} 
                className="relative min-h-screen font-stt flex flex-col lg:flex-row lg:justify-between lg:items-end gap-10.5 px-8 pb-10.5 bg-amblue *:text-amwhite overflow-hidden"
            >
                {/* --- LOGO ABSOLUT DI TENGAH BACKGROUND --- */}
                <svg 
                    className="absolute inset-0 m-auto w-auto h-[70vh] max-w-full overflow-visible pointer-events-none opacity-20"
                    viewBox="0 0 302 215" 
                    fill="none" 
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path d={logoPath} fill="#F3AE41" />
                </svg>

                {/* --- KONTEN FOOTER --- */}
                <div className="relative z-10 flex flex-col gap-6 w-auto mt-auto lg:mt-0 pointer-events-none">
                    <h1 className="footer-item font-stn text-aml/[120%] lg:text-am2xl font-bold will-change-transform">
                        Interested to<br/>collaborate? Let’s<br/>get in touch!
                    </h1>
                    
                    <form 
                        onSubmit={handleSubmitEmail} 
                        className="footer-item flex flex-col gap-2 will-change-transform pointer-events-auto"
                    >
                        <label htmlFor="email" className="text-sm font-bold font-stn">
                            Email
                        </label>
                    ...
                    <div className="footer-item flex flex-col gap-2 -mb-4 will-change-transform pointer-events-auto">
